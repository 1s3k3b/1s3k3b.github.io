...
class Point {
    ...
    render(ctx) {
        this.y = this.y + Math.floor(Math.sqrt(this.size));
        ctx.beginPath();
...
const player = {
    ...
    render(ctx) {
        ...
        if (this.keyPressed === "r" && this.x + this.w <= window.innerWidth - 5) this.x += 5;
        if (this.keyPressed === "l" && this.x >= 5) this.x -= 5;
        ctx.fillStyle = this.color;
...
window.onload = () => {
    ...
    setInterval(() => {
        ...
        ctx.fill();
        player.render(ctx);
        if (Math.random() > 0.8) points.push(new Point());
