...
class Enemy {
    ...
    render(ctx) {
        ...
        if (this.moving) {
            if (this.moving === -1 && this.y > 0) this.y -= 0.5;
            else if (this.y + 40 < window.innerHeight) this.y += 0.5;
        }
        ctx.beginPath();
...
window.onload = () => {
    ...
    setInterval(() => {
        ...
        if (Math.random() > 0.9966) enemies.push(new Enemy(window.innerWidth, Math.floor(Math.random() * window.innerHeight)));
